<template>
  ...
  <button @click="logout" class="main-button back">Wyloguj</button>
    ...
    <form v-if="isLoggedIn" @submit.prevent="addEntry">
      <div class="user-input-box">
        <h4>Wypaliłeś łącznie: {{ user.totalCigarettes }}</h4>
        <br />
...
export default {
  ...
  computed: {
    ...
    user() {
      ...
      if (user) {
        const uniqueDays = new Set(user.history.map((entry) => entry.date))
          .size;
...
.add {
  background-color:rgb(24, 62, 168);
}
...
td,
th {
  border: 1px solid rgb(142, 141, 141, 0.3);
  padding: 10px;
}
